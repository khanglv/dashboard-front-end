...
class DialogAction extends Component{
    ...
    _onOrderActionInvest = async(actionType)=>{
        try {
            ...
            if(lstIndexs.length > 0){
                ...
                for(let i = 0; i < lstIndexs.length; i++){
                    ...
                    try {
                        if(actionType && actionType === 'TAICANBANGDANHMUC'){
                            const res = await this.actionOrderApi(lstIndexs[i], lstIndexs[i].type);
                ...
                await this.actionOrderPortfolioApi(lstOrdered, actionType);
    ...
    actionOrderApi = async(orderItem, type = null) => {
        const {
            accountInfo
        } = this.state;
    ...
    actionOrderPortfolioApi = async(data, actionType)=>{
        try {
            if(data.length > 0){
                let typeTmp = 'BAN';
                if(actionType && actionType === 'TAICANBANGDANHMUC'){
                    typeTmp = 'TAICANBANGDANHMUC';
                }
                let subNumber = '00';
                let obj = JSON.parse(localStorage.getItem('rememberSectionOrder'));
                if (obj) {
                                ...
                                {lstIndexs.length > 0 ?
                                    <List
                                        ...
                                        <List.Item className="item">
                                            ...
                                            <Col span={4}>
                                                {
                                                    this.props.actionType === 'TAICANBANGDANHMUC' ? 
                        ...
                        <div className="footerRoot">
                            {
                                ...
                                <Row>
                                    ...
                                    <Col span={12} className="right isBtnGeneral" onClick={()=> this._onOrderActionInvest(this.props.actionType)}>
                                        ĐẶT LỆNH
                                    </Col>
                                </Row>
                            }
...
export default withRouter(connect(mapStateToProps, mapDispatchToProps) (DialogAction));
