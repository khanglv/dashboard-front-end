...
class OrderBuy extends Component{
    ...
    componentDidUpdate(prev){
        try {
            ...
            if(this.props.exchangeOnline && prev.exchangeOnline !== this.props.exchangeOnline){
                let objExchange = this.props.exchangeOnline;
                //changeLastStockCode
                if(Object.keys(objExchange.changeLastStockCode).length > 0){
                    let obj = objExchange.changeLastStockCode;
                    this.setState({
                        priceOrder: obj.last,
                        isWarningPrice: 0
                    });
                ...
                if(Object.keys(objExchange.changeStockCode).length > 0){
                    let obj = objExchange.changeStockCode;
                    this.setState({
                ...
                //estimatedData
                if(Object.keys(objExchange.estimatedData).length > 0){
                    let obj = objExchange.estimatedData;
                    this.setState({
            ...
            if(this.props.stockCodeChange && prev.stockCodeChange !== this.props.stockCodeChange){
                ...
                socket.disconectChanel(this.props.stockCodeChange);
                this.props._onChangeCodeStock(this.props.stockCodeChange);
                storage.rememberSectionOrder(JSON.stringify({CODE_STOCK: this.props.stockCodeChange, SUB_NUMBER: subNumber}));
            }
        } catch (error) {
            console.log("catch");
        }
                                ...
                                <Col span={12}>
                                    ...
                                    <span className={customSelectOrderNumber} ref={this.wrapperRefPrice}
                                        ...
                                        <div style={{position: 'relative'}}>
                                            {
                                                [2,3,4,5].includes(isWarningPrice) === true ? 
